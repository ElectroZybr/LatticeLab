...
bool FontManager::load(float uiScale) {
    ...
    main = fonts->AddFontFromFileTTF("assets/fonts/Rubik-VariableFont_wght.ttf", scaledSize(37.5f), &mainConfig);
    if (!main) {
        return false;
    }
    ...
    if (!fonts->AddFontFromFileTTF("assets/fonts/Font Awesome 5 Free-Solid-900.otf", scaledSize(30.0f), &iconConfig, iconRanges)) {
        return false;
    }
    ...
    icons = fonts->AddFontFromFileTTF("assets/fonts/Font Awesome 5 Free-Solid-900.otf", scaledSize(22.5f), &sideIconConfig, iconRanges);
    if (!icons) {
        return false;
    }
    ...
    dialog = fonts->AddFontFromFileTTF("assets/fonts/Rubik-VariableFont_wght.ttf", scaledSize(18.0f), &dialogConfig, ruRanges);
    if (!dialog) {
        return false;
    }
