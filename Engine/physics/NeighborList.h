...
public:
    ...
    [[nodiscard]] std::span<const size_t> neighborsIndices(size_t atomIndex) const { 
        auto r = rangeFor(atomIndex);
        return std::span(neighbors_).subspan(r.first, r.second - r.first); 
    };
    [[nodiscard]] const std::vector<size_t>& neighbors() const { return neighbors_; }
    [[nodiscard]] const std::vector<size_t>& offsets() const { return offsets_; }
